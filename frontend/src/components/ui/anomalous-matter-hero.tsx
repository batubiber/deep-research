...
export function GenerativeArtScene({ cameraZ = 3 }: { cameraZ?: number } = {}) {
  ...
  useEffect(() => {
    ...
    const threeColor = new THREE.Color(0xffffff);
    ...
    >
      ...
      <div className="relative z-20 flex flex-col items-center justify-end h-full pb-20 md:pb-32 text-center">
        <div className="max-w-3xl px-4 animate-fade-in-long">
          <h1 className="text-sm font-mono tracking-widest text-[var(--color-primary)] uppercase">
            {title}
          </h1>
          <p className="mt-4 text-3xl md:text-5xl font-bold leading-tight text-[var(--color-text-primary)]">
            {subtitle}
          </p>
          <p className="mt-6 max-w-xl mx-auto text-base leading-relaxed text-[var(--color-text-secondary)]">
            {description}
          </p>
        </div>
      </div>
